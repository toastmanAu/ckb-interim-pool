...
## Dev-chain payout drill — definition-of-done (2026-08-12)

`deploy/ckb-dev-test.sh` + `deploy/drill-devchain.sh`: the complete chain on a
local CKB dev chain (nervos/ckb:v0.118.0, dummy PoW, `cellbase_maturity=0`,
epoch length 10):
  **miner-sim → edge → shares/blocks → NATS → ingest → canonical → mature →
  PPLNS allocation → immutable ledger → signed payout tx → node accepted.**

A run: 614 blocks mined; 343 settled to ledger (6,822,867,082,784,989
shannons credited, conservation exact); 236 orphans (expected at sim speed —
correctly never credited); payout batch reserved; real signed transaction
broadcast and accepted (`send_transaction` succeeded, mempool pending until
mining resumes).

New capability: **`src/payout/ckb-tx-builder.js`** — fully self-contained
CKB payout construction/signing (no ckb-cli dependency):
- cell collection by scanning the pool's own cellbases (no indexer needed);
- RFC 0022 sighash_all message — `blake2b(txHash(rawTx) || u64le|witness …)`
  with a WitnessArgs 65-zero lock placeholder (verified against
  ckb-system-scripts + lumos reference implementations — this was the bug:
  hashing the full serialized tx with a zeroed placeholder instead);
- WitnessArgs lock witness (modern ckb-lib scripts), secp256k1 recovery id
  resolved by pubkey match;
- cell deps resolved from the genesis (dep group on mainnet; the dev chain
  has NO dep group cells — code deps for the code + data cells, matched by
  type-script hash / data hash);
- private keys in-process only (payout host); dev key lives in
  /tmp/opencode/pool-key.json (generated by ckb-dev-test.sh).

Errors decoded along the way (-2 parse, -31 pubkey mismatch) via the official
